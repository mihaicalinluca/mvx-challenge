...
const lib_PATH_EXPR: &str = "file:output/lottery-sc.wasm";

fn world() -> ScenarioWorld {
    let mut blockchain = ScenarioWorld::new();
    blockchain.set_current_dir_from_workspace("contracts/examples/lib");

    blockchain.register_contract("file:output/lottery-sc.wasm", lottery_sc::ContractBuilder);
    blockchain
}
...
fn lib_whitebox() {
    let mut world = world();
    let lib_whitebox = WhiteboxContract::new("sc:lib", lottery_sc::contract_obj);
